...
Setting \texttt{sharingType} to \texttt{.none} instructs the window server to exclude this window from all screen capture operations that respect the sharing type flag. On macOS 14--26, this includes both legacy CoreGraphics capture and the system-level capture APIs that underlie ScreenCaptureKit-based tools.
...
As shown in Table~\ref{tab:evasion}, the attack achieved a 100\% evasion rate on all tested platforms. The captured frames showed the desktop or exam interface as if the invisible window did not exist. No visual artefacts (black rectangles, compositing errors, flicker) were observed in any captured frame across over 10,000 frames analysed per configuration. Fig.~\ref{fig:windows-diff} shows a pixel-level forensic diff of the Windows capture, and Fig.~\ref{fig:macos-capture} shows the macOS capture comparison.
...
\item \textbf{Vendor notification} (January 2026): Affected proctoring vendors (Proctorio, ProctorU, Respondus) were notified with a detailed technical report and a 90-day disclosure window. At the time of writing, no vendor has confirmed deployment of a mitigation targeting the display affinity attack vector specifically.
...
\item \textbf{OS vendor communication} (February 2026): Microsoft and Apple were informed of the security implications of their display affinity APIs in the proctoring context via their respective security reporting channels.
\item \textbf{Public disclosure} (March 2026): This paper is published after the 90-day disclosure window, allowing vendors time to develop and deploy mitigations.
\end{enumerate}
...
Following coordinated disclosure to OS vendors, both Apple Product Security and the Microsoft Security Response Center (MSRC) responded formally. Table~\ref{tab:disclosure} summarises the disclosure timeline and outcomes across all contacted parties.

\begin{table}[t]
\centering
\caption{Coordinated Disclosure Summary}
\label{tab:disclosure}
\begin{tabular}{p{1.4cm}p{1.0cm}p{1.1cm}p{3.7cm}}
\toprule
\textbf{Party} & \textbf{Notified} & \textbf{Response} & \textbf{Classification} \\
\midrule
Apple Product Security & Mar.\ 2026 & Mar.\ 2026 & Consistent with documented functionality; not a security issue \\[4pt]
Microsoft MSRC & Feb.\ 2026 & Apr.\ 2026 & By-design behaviour; not a security vulnerability \\[4pt]
Proctorio & Jan.\ 2026 & Ongoing & Acknowledged; implementing behavioural blocks \\[4pt]
ProctorU   & Jan.\ 2026 & ---      & No response at time of writing \\[4pt]
...
While proctoring serves academic integrity~\cite{lee2022online,noordegraaf2023ethical}, research consistently shows it imposes psychological costs including increased anxiety and decreased performance~\cite{conijn2022fear,ruth2024cheating,gribbins2023exploration}. Structural bypasses like the Invisible Window show that proctoring creates a false sense of security while imposing real costs on test-takers~\cite{khalil2022nexus,prinsloo2024vulnerable}.
